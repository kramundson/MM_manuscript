...
## Figure S1: Chromosome number by maternal parent. Include data from previous studies.
```{r}
# make data frame that groups by non inducer and chromosome number
prop_data <- fig2c_data %>% 
...
plt <- prop_data %>% 
  ggplot(.) +
  geom_bar(aes(x = reorder(Non_inducer, -frac_25), y = n, fill = factor(chrnum)), stat = "identity", position = position_fill(reverse = T), width = 0.8) +
...
grid.newpage()
grid.draw(p)
ggsave("2020-10-23-aneu-by-nonHI.png", plot = p, width = 6, height = 6, units = "in", device = "png")
```
...
## Figure S2: Chromosome gains and losses by homolog
```{r}
fig_S2_data <- with_cen %>% 
  filter(snp_ploidy == "2x" | is.na(snp_ploidy)) %>%
  filter(Monica_ploidy == "2x" | is.na(Monica_ploidy)) %>% 
  filter(!is.na(ctrl_CIP)) %>%
  filter(manual_fp != "fp") %>% 
...
fig_S2
ggsave("2020-10-23-fig-S2.png", plot = fig_S2, width = 10, height = 4, units = "in", device = "png")
```

```{r}
table(fig_S2_data$chrom,fig_S2_data$`Chromosome State`)
```
...
## Figure S3: Combined dosage/SNP plots of eight HI addition dihaploids:
```{r}
fig_S3 <- ggarrange(stack_plots("MM246", with_cen, with_cen),
...
fig_S3
ggsave("2020-10-23-fig-S3.png", plot = fig_S3, width = 16, height = 16, units = "in", device = "png")
```