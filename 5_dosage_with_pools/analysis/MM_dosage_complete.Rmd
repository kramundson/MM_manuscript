...
fig2a <- fig2a_data %>% 
  ...
  ggplot(., aes(x = sample_num_recode, y = chrom_normcov)) +
    ...
    geom_vline(xintercept = mons, color = "gray70") +
    geom_point(size = 0.5) +
    geom_text(data = filter(fig2a_data, chrom_normcov <= cut_lo), aes(label = chromshort), size = 3.5, color = "black") +
    geom_text(data = filter(fig2a_data, chrom_normcov >= cut_hi & UCD_Sample_Code != "MM1114"), aes(label = chromshort), size = 3.5, color = "black") +
    ...
    annotate("text", x=207, y=2.7, label="3", size=4, color = "black") + # chromosome 3
    annotate("segment", x=209, y=2.82, xend=217, yend=2.646788, linetype = "dashed") + # chromosome 8
    annotate("text", x=207, y=2.84, label="8", size=4, color = "black") + # chromosome 8  
    annotate("segment", x=209, y =2.54, xend=217, yend=2.618737, linetype = "dashed") + # chromosome 9
    annotate("text", x =207, y =2.55, label="9", size=4, color = "black") + # chromosome 9
...
fig2b_data <- with_cen %>%
  filter(sample_tot >= 2e4) %>% 
  # filter(snp_ploidy == "2x" | is.na(snp_ploidy)) %>% # changed per reviewer request
  # filter(Monica_ploidy == "2x" | is.na(Monica_ploidy)) %>% # changed per reviewer request
  filter(snp_ploidy == "2x") %>% 
...
fig2b <- ggplot(fig2b_data, aes(x = factor(chrnum), fill = factor(chrnum))) +
  geom_bar() +
  geom_text(stat='count', aes(label=..count..), vjust=-0.2, size = 3.5, color = "black") +
...
fig2d <- ggplot(fig2d_data, aes(x = factor(chrnum), fill = factor(Parent_assoc_aneuploidy))) +
    ...
    geom_text(stat='count', aes(label=..count..), vjust=-0.4, position = position_dodge2(preserve = "single", width = 0.9), size = 3.5, color = "black") +
...
# ggsave("2021-01-06-fig2.png", plot = fig2, width = 7, height = 5, units = "in", device = "png")
# ggsave("fig2.svg", plot = fig2, width = 7, height = 5, units = "in", device = "svg") # segments not drawn correctly
ggsave("fig2.eps", plot = fig2, width = 7, height = 5, units = "in", device = "eps") # segments drawn correctly; use this
```

```{r}
fig2_com <- ggarrange(
  ggarrange(fig2a, fig2c,  align = "hv", nrow = 2, ncol = 1, hjust = 0, vjust = 1.5, widths = c(1,1)),
  ggarrange(fig2b, fig2d,  align = "hv", nrow = 2, ncol = 1, hjust = 0, vjust = 1.5, widths = c(1,1)),
  nrow = 1,
  ncol = 2,
  heights = c(1,1),
  widths = c(2.5,1)
...
fig2_com
ggsave("fig2_com.png", plot = fig2_com, width = 9, height = 5, units = "in", device = "png")
```

```{r}
fig2_committee_part2 <- ggarrange(fig2b, fig2d, align = "v", nrow = 2, ncol = 1)
ggsave("fig2b_committee_part2.png", plot = fig2_committee_part2, width = 3, height = 4, units = "in", device = "png")# ggsave("fig2d_committee.png", plot = fig2d, width = 4, height = 3, units = "in", device = "png")
```
...
fig_S1_data <- with_cen %>% 
  # filter(snp_ploidy == "2x" | is.na(snp_ploidy)) %>% # removed per reviewer request
  # filter(Monica_ploidy == "2x" | is.na(Monica_ploidy)) %>% # removed per reviewer request
  filter(snp_ploidy == "2x") %>% # only keeps the dihaploids for which we had SNP information
  filter(!is.na(ctrl_CIP)) %>%
  filter(manual_fp != "fp") %>% 
  filter(sample_tot >= 2e4) %>% 
...
# ggsave("2021-01-06-figS1.png", plot = fig_S1, width = 10, height = 4, units = "in", device = "png")
ggsave("figS1.svg", plot = fig_S1, width = 10, height = 4, units = "in", device = "svg")
```
...
After removing the SNP set, chisq is now 18.39.
...
```{r}
# post hoc chromosome gains
fig_S1_data %>% 
  filter(`Chromosome State` == "Gain") %>% 
  group_by(chrom) %>% 
  tally() %>% 
  select(n) %>% 
  chisq.test()
...
### Figure S3: Chromosome number by HI parent:
```{r}
propHI <- fig2b_data %>% 
...
# ggsave("2021-01-06-fig-S2.png", plot = p2, width = 6, height = 3, units = "in", device = "png")
ggsave("figS3.png", plot = p2, width = 6, height = 3, units = "in", device = "png")
ggsave("figS3.svg", plot = p2, width = 6, height = 3, units = "in", device = "svg")
```

### Figure S2: Chromosome number by maternal parent. Include data from previous studies.
```{r}
...
# ggsave("2021-01-06-fig-S3.png", plot = p, width = 6, height = 6, units = "in", device = "png")
ggsave("figS2.svg", plot = p, width = 6, height = 6, units = "in", device = "svg")
```
...
ggsave("figS4.eps", plot = fig_S4, width = 16, height = 16, units = "in", device = "eps")
```
...
ggsave("fig3_top.eps", plot = fig3_top, width = 12, height = 5, units = "in", device = "eps")
...
ggsave("fig3_bottom.eps", plot = fig3_bottom, width = 12, height = 5, units = "in", device = "eps")
```
...
hybs_long %>% 
  filter(only_parental_origin_unknown == "known") %>% 
  filter(snp_ploidy %in% c("3x", "4x")) %>% 
  dplyr::select(UCD_Sample_Code, Non_inducer, Inducer, snp_ploidy, parent_summary, Phureja_marker_phenotype) %>% 
  distinct() %>% 
  group_by(Phureja_marker_phenotype, snp_ploidy) %>% 
  tally %>% 
  arrange(snp_ploidy, Phureja_marker_phenotype)
```
...
### Extract hybrids for logistic regression model of HI-derived genome instability:
```{r}
hybs_for_model <- hybs_long %>% 
  filter(snp_ploidy %in% c("3x", "4x")) %>% 
  group_by(UCD_Sample_Code) %>% 
  mutate(paternal_unstable = ifelse(paternal_lesion_count >= 1, 1, 0),
         maternal_unstable = ifelse(maternal_lesion_count >= 1, 1, 0)) %>% 
  dplyr::select(UCD_Sample_Code, snp_ploidy, paternal_unstable, maternal_unstable,
                parent_summary, Phureja_marker_phenotype,
                Inducer, Non_inducer) %>% 
  distinct() %>% 
  ungroup() %>% 
  mutate(Inducer = factor(Inducer, levels = c("PL4", "IVP35", "IVP101"))) %>% 
  rename(ploidy = snp_ploidy)
head(hybs_for_model)
```

### Logistic regression model of HI genome instability:
```{r}
result <- glm(paternal_unstable ~ ploidy + maternal_unstable + Inducer, data = hybs_for_model, family = binomial("logit"))
summary(result)
anova(result)
confint.default(result)
wald.test(b = coef(result), Sigma = vcov(result), Terms=1)
```

> Significant effect of ploidy (more likely to have paternal chromosome instabililty if tetraploid).
Effects of maternal aneuploidy and HI genotype not significant.

> Estimated ploidy effect size: 3.02579, (95% CI: [1.513420,  4.5381682]).

> How well does the model fit?
If the model is correct and when ni's are large,
the residual deviance D has a chi-square distribution approximately:

residual D ~ X**2 dfresid

```{r}
pchisq(154.84, df=131, lower.tail = F)
```

> Here, the large residual deviance indicates a bad fit. Typical diagnostics:
1. Correct predictors may not be included.
2. Predictors not transformed appropriately (not applicable with categorical predictors).
3. Outliers or points with undue influence.
4. Small predictors: residual deviance not chi-square distributed so it is useless for assessing goodness of fit.
...
> Here, data look nearly bimodal. This may explain the bad fit.